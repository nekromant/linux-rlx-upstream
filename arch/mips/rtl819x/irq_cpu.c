...
{
 	set_c0_status(0x100 << (d->irq - BSP_IRQ_CPU_BASE));
	irq_enable_hazard();
}

static inline void mask_rlx_irq(struct irq_data* d)
{
	clear_c0_status(0x100 << (d->irq - BSP_IRQ_CPU_BASE));
	irq_disable_hazard();
}
...
void __init rlx_cpu_irq_init(int irq_base)
{
	...
	for (i = irq_base + 2; i < irq_base + BSP_IRQ_CPU_NUM; i++)
		irq_set_chip_and_handler(i, &rlx_cpu_irq_controller, handle_percpu_irq);
